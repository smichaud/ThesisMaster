...
\todo{includes a self- similarity analysis, (used to prevent false positives in area where very few distinctive structure like corridor)}
\todo{Given a database of 3D scans and a scan as input query, our algorithm returns a set of scans which are potential matches with the input. Additionally, it calculates for every returned scan a transformation and a score reflecting how certain the system is that the scans actually match}
...
\begin{algorithm}
    \begin{algorithmic}[1]
        ...
        \State
        \State $initSimilarities \gets \emptyset$ \Comment{Set of (scan reference, initial similarity score)}
        ...
        \ForAll{$transfo \in possibleTransformations$}
        \State $transfoScore \gets \textsc{computeTransformationScore}(transfo)$
        \State
        \If{$transfoScore \geq acceptanceThreshold$}
        \If{$transfoScore \geq bestTransfoScore$}
...
\todo{Little paragraph for details. The similarity is based on manathan or (euclidean) dist. The bow done by k-means.  Says that features from bow and correspondences are differents and create a Table~\ref{tab:chap_slam_narf_parameters} with the parameters}
\begin{itemize}
...
\begin{table}[H]
    ...
    \begin{tabular}{@{}lll@{}}
        ...
        \bottomrule
    \end{tabular}
    \caption{\todo{Describe what parameters means, }}
