...
\todo{Working on that paragraph}
In Algorithm~\ref{alg:chap_slam_overview}, \gls*{bow} descriptors are computed at line~\ref{alg:bow_descriptors} and features are computed later at line~\ref{alg:test}. These two steps are both based on NARF features, but they use different parameters. Authors explain that: \enquote{For the BoW approach a high number of features describing small parts of the environment is most useful\dots However, when matching a new query [scan] $z*$ against D [the database], a smaller number of more distinctive features is needed}. Table~\ref{tab:chap_slam_narf_parameters} shows the feature parameters used for those two steps.

In Algorithm~\ref{alg:chap_slam_overview} the \textsc{createDictionaryForBagOfWord} function create 200 words (i.e clusters) using k-means. Using the output dictionary,

\todo{The similarity is based on (narf=manathan) or (bow=euclidean) dist.}
\todo{each descriptor encore 3d transform, ordered by descriptor distance to find best transformations}
\todo{Say that the scoring method is described in Section 3 E, but is to long to detail here}
...
\begin{algorithm}
    \begin{algorithmic}[1]
        ...
        \State $allBowDescriptors \gets \textsc{calculateAllDescriptorsForBagOfWords}(scanDatabase)$ \label{alg:bow_descriptors}
        \State $dictionary \gets \textsc{createDictionaryForBagOfWords}(allBowDescriptors)$
        \State $newScanBow \gets \textsc{computeBagOfWords}(newScan, dictionary)$ \Comment{Keep scan reference}
        ...
        \State
        \State $potentialMatches \gets \emptyset$, $i \gets 1$
        \State $newScanFeatures \gets \textsc{getScanFeatures}(newScan)$
        \While{$time \leq timeout$ \textbf{and} $i \leq |scanDatabase|$}
        \State $scan \gets \textsc{getScan}(sortedSimilarities_i)$
        \State $scanFeatures \gets \textsc{getScanFeatures}(scan)$ \Comment{Each feature encodes the 3D pose} \todo{max 2000}
        \State $sortedMatches \gets \textsc{getSortedFeatureMatches}(scanFeatures, newScanFeatures)$ \label{alg:ordered_matches}

        \State
        \State $bestTransfoScore = -\infty$, $bestTransfo \gets null$
        \ForAll{$match \in sortedMatches$} \Comment{Following ascending order}
        \State $transfo,score \gets \textsc{computeMatchTransfoAndScore}(match)$

        \State
        \If{$score \geq scoreAcceptanceThreshold$}
        \If{$score \geq bestTransfoScore$}
